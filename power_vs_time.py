...
import os
import csv
import optics.powerdetector
import optics.setup_constants as hw

class GUI:
    def __init__(self, master, powermeter):
    ...
    def read_power(self):
        return self._powermeter.read_power()
    ...
    def record(self, event=None):
        self._power.set(self.read_power())
        self.fetch(event)
...
with optics.powerdetector.connect(hw.power_detector_address) as powermeter:
    root = tk.Tk()
    q = GUI(root, powermeter)
    q.make_gui()
    root.mainloop()